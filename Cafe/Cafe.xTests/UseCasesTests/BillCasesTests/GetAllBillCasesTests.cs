...
public class GetAllBillCasesTests
{
    ...
    public static GetAllBillQueryHandler handler = new GetAllBillQueryHandler(mapper, billRepository);

    [Fact]
    public async void GetAllTest1()
    {
        var request = new GetAllBillQuery(0, 10);

        var result = (OperationResult<List<GetBillQueryResponse>>)(await handler.Handle(request, default(CancellationToken)));
    ...
    {
        var request = new GetAllBillQuery(1, 10);
